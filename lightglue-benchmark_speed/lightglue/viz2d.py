...
def add_text(
    idx,
    text,
    output_dir, query_img, class_img, n_matching_kpts,
    pos=(0.01, 0.99),
    fs=10,
    color="w",
    lcolor="k",
    ...
    ax = plt.gcf().axes[idx]
    t = ax.text(
        *pos, "Matching kpts: " + str(n_matching_kpts), fontsize=fs, ha=ha, va=va, color="red", transform=ax.transAxes
    )
    ...
    # t = ax.text(
    #     *(0.01, -0.99), "Matching kpts: " + str(n_matching_kpts), fontsize=15,  color="blue", transform=ax.transAxes
    # )
    # if lcolor is not None:
    #     t.set_path_effects(
    #         [
    ...
    #         ]
    #     )
    output_dir = os.path.join(output_dir, query_img)
    plt.savefig(os.path.join(output_dir, class_img))
    plt.close()
